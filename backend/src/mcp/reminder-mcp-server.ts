...
// scheduler-service.ts's createScheduledAction already models. What
// actually happens when the reminder fires is still a stub
// (scheduler-service.ts's executeScheduledAction 'lead_reminder' case) --
// the real, testable part here is that a live EventBridge schedule gets
// created for this specific lead at the requested time. Shared by both the
// MCP tool below and journey-executor-service.ts's direct in-process
// dispatch, same pattern as booking-mcp-server.ts.
...
export function createReminderMcpServer(): McpServer {
  ...
  server.registerTool(
    'schedule_reminder',
    {
      ...
      description:
        'Schedules a one-off reminder for a specific lead at a future wall-clock time, via a real EventBridge Scheduler schedule. What happens when it fires is not yet implemented (no notification infra exists).',
      inputSchema: {
        botId: z.string(),
        clientId: z.string(),
        leadId: z.string(),
