...
// PUT URL (kb-service.ts's KB_FILE_CONTENT_TYPES) -- S3 rejects the upload if
// the header sent here doesn't match what was signed, so this can't just be
// the browser-reported file.type.
...
// Matches useTierCheckout.ts's payment-status polling convention (3s interval,
// 10 attempts / ~30s cap) rather than useIndexingStatus's uncapped 2.5s loop.
const POLL_INTERVAL_MS = 3000
const POLL_MAX_ATTEMPTS = 10
const ACTIVE_FILE_STATUSES: ReadonlySet<KnowledgeBaseEntry['indexingStatus']> = new Set(['queued', 'processing'])
...
export default function KnowledgeBasePage() {
  ...
  // endpoint -- no lighter per-entry route exists) rather than one poller per
  // entry. Restarts (and resets its attempt budget) whenever the set of
  // active entryIds changes -- a status flip or a fresh upload both count.
  ...
  }, [botId, activeEntryIds, refreshNonce])
  ...
  async function startUpload(file: File, fileType: KBFileType) {
    ...
    try {
      const urlRes = await getKBUploadUrl({ botId, filename: file.name, fileType, fileSizeBytes: file.size })
      ...
      const confirmRes = await confirmKBUpload({
        botId,
        entryId,
        filename: file.name,
        fileType,
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-3 mt-3">
                <a
                  href="mailto:support@vyostra.com?subject=Upgrade my Vyostra AI plan"
