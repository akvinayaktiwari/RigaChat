...
import type { LeadState, LeadStatus, UnifiedLead, UrgencyTier } from '../types/index'
...
// urgencyTier is required on UnifiedLead because the server now stamps the
// tier and sends it on the wire -- it did not when these tests were first
// written on a branch that never landed. It is filled here with the same
// classification the server applies (serverTier below, inverted back to a
// name), so the fixture cannot assert against a tier the server would never
// have sent. leadUrgency does not read the field; the cross-check does.
const TIER_NAMES: UrgencyTier[] = ['overdue', 'untouched', 'scheduled', 'in_progress', 'closed']

function lead(state: Partial<LeadState> | null, createdAt = at(0)): UnifiedLead {
  const resolved = state === null ? null : ({ status: 'new', notes: [], ...state } as LeadState)
  const partial = {
    leadId: 'lead-1',
    clientId: 'client-1',
    source: 'chat' as const,
    leadRef: { source: 'chat' as const, botId: 'bot-1', leadId: 'lead-1' },
    createdAt,
    state: resolved,
  }
  return { ...partial, urgencyTier: TIER_NAMES[serverTier(partial as UnifiedLead, NOW)] }
}
...
// stops being a queue.
//
// TIER_* is a hand-copy of lead-inbox-service.ts's TIER_RANK and
// urgencyTierOf (re-checked against them on 2026-09-14; the line numbers this
// comment used to cite had already moved). Be honest about what
