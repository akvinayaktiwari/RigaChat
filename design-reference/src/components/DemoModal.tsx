...
export default function DemoModal({ isOpen, onClose }: DemoModalProps) {
  ...
  // Simulated Chat Stream for the video
  ...
  const script: { triggerProgress: number; msg: ChatMessage }[] = [
    {
      triggerProgress: 30,
      msg: { id: "2", sender: "bot", text: "Absolutely! 🕶️ Here are our top wooden-framed sunglasses crafted from recycled maple. They are light-weight and polarized.", time: "12:40 PM" }
    },
    {
      triggerProgress: 45,
      msg: { id: "3", sender: "bot", text: "🔥 Special Offer: Buy today and get 15% off with code RIGA15. Would you like me to add them to your cart?", time: "12:41 PM" }
    },
    {
      triggerProgress: 65,
    ...
    {
      triggerProgress: 80,
      msg: { id: "5", sender: "bot", text: "Added! 🎉 Your cart is updated. Total: $67.15 (Saved $11.85). Tap below to check out instantly via Apple Pay or Card.", time: "12:41 PM" }
    }
  ];
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setProgress((prev) => {
        ...
        // Check if we should insert a message from the script
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-4 bg-on-background/80 border-b border-outline/10 text-white">
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold text-sm uppercase tracking-wider text-outline-variant">RigaChat Live Interactive Demo</span>
              </div>
              <button
                onClick={onClose}
            ...
            <div className="grid lg:grid-cols-12">
              {/* Left Sidebar controls inside demo */}
              <div className="lg:col-span-3 bg-on-background/40 p-4 border-r border-outline/10 flex flex-col gap-2">
                ...
                <button
                  onClick={() => { setActiveTab("chat"); handleReplay(); }}
                ...
                <div className="mt-auto pt-4 border-t border-outline/10 hidden lg:block">
                  <div className="bg-primary/10 border border-primary/20 p-3 rounded-lg">
                    ...
                    <p className="text-[10px] text-outline-variant mt-0.5">Captured dynamically by RigaChat intelligence engine.</p>
                  </div>
                </div>
              </div>

              {/* Central Main Video Display */}
                ...
                {activeTab === "chat" ? (
                    ...
                    <div className="flex-1 border border-outline/15 rounded-xl overflow-hidden bg-surface flex flex-col">
                      ...
                      <div className="bg-surface-container h-10 px-4 border-b border-outline-variant/30 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          />
                          <span className="text-xs font-bold text-on-surface">Rachel · RigaChat Sales Bot</span>
                        </div>
                      ...
                      <div className="flex-1 p-4 space-y-3 overflow-y-auto max-h-[220px]">
                        <AnimatePresence>
                          {messages.map((m) => (
                            ...
                            >
                              <div className={`max-w-[80%] rounded-xl px-3 py-2 text-xs ${m.sender === "customer" ? "bg-primary text-white rounded-tr-none" : "bg-surface-container text-on-surface rounded-tl-none border border-outline-variant/30"}`}>
                                <p className="leading-relaxed">{m.text}</p>
                                <span className={`text-[9px] block text-right mt-1 ${m.sender === "customer" ? "text-white/70" : "text-on-surface-variant"}`}>{m.time}</span>
                              </div>
                            </motion.div>
                          ))}
                        </AnimatePresence>
                  ...
                  <div className="flex-1 flex flex-col justify-center items-center text-center px-4">
                    ...
                    <p className="text-xs text-outline-variant mt-2 max-w-sm">
                      RigaChat tracks cart abandonments and automatically engages visitors before they leave the page. It can offer discount triggers or real-time support, resulting in up to 30% recovery rate.
                    </p>
                    <button
